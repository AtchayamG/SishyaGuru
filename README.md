...
## Hosted Replay demo (GitHub Pages)

A static, credential-free Replay-only build deploys to
<https://atchayamg.github.io/SishyaGuru/> (pending first deployment from `main`).
It is generated by `npm run build:pages`, which builds a static export with
`GITHUB_PAGES=true` and verifies that `out/index.html` uses the `/SishyaGuru`
base path and pins Replay mode. The static site never offers Live mode and
contains no API routes or credentials. Deployment runs automatically via
